...
live2d_settings['canSwitchTextures']    = false;         // 显示 衣服切换    按钮，可选 true(真), false(假)
...
const  initModel = function initModel(waifuPath, type) {
  ...
  if(live2d_settings.loadLocalModel) {
    if (!live2d_settings.modelStorage || modelName == null) {
      var modelName = live2d_settings.modelName;
      var modelTexturesId = live2d_settings.modelTexturesId;
    }
    loadLocalModel(modelName, modelTexturesId);
  } else {
...
function loadLocalModel(modelName, modelTexturesId=0) {
  if (live2d_settings.modelStorage) {
...
function loadTipsMessage(result) {
  ...
  function loadOtherModel() {
    if(live2d_settings.loadLocalModel) {
      var modelName = modelStorageGetItem('modelName');
      var index = live2d_settings.localModelNames.findIndex(function(item) {
        return item === modelName;
    });
    var newNameIndex = index + 1;
    if(newNameIndex >= live2d_settings.localModelNames.length) {
      newNameIndex = 0
    }
    
    modelName = live2d_settings.localModelNames[newNameIndex]
    loadLocalModel(modelName);
    var message = waifu_tips.model_message.default;
    $.each(waifu_tips.model_message, function(i,val) {if (i==modelName) message = getRandText(val)});
